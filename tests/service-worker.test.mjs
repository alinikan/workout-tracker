...
test("an HTTP error cannot replace the last working offline page", async () => {
  ...
  const cache = await app.caches.open("recomp-gym-console-v33");
...
test("activation retains one prior app version and leaves unrelated caches alone", async () => {
  ...
  for (const name of ["other-app", "recomp-gym-console-v31", "recomp-gym-console-v32", "recomp-gym-console-v33"]) await app.caches.open(name);
  await app.activate();
  assert.deepEqual([...app.stores.keys()].sort(), ["other-app", "recomp-gym-console-v32", "recomp-gym-console-v33"]);
});
